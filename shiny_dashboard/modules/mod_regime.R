  ...
  tabsetPanel(
    ...
    tabPanel("Per Ticker",
      tagList(
        ...
        div(
          style = paste0(
            "overflow-y:auto; height:calc(100vh - 342px); background:", DARK_BG, ";"
          ),
    ...
    tabPanel("Fingerprint",
      layout_columns(
        ...
        card(
          full_screen = TRUE,
          card_header("US Sectors — Regime Fingerprint (Boxplot)",
                      style = paste0("background:", DARK_HDR, "; color:", TEXT_DIM,
                                     "; font-size:11px; padding:5px 12px;")),
          plotOutput(ns("fp_sector"), height = "calc(100vh - 360px)")
        ),
        card(
          full_screen = TRUE,
          card_header(textOutput(ns("fp_filtered_title"), inline = TRUE),
                      style = paste0("background:", DARK_HDR, "; color:", TEXT_DIM,
                                     "; font-size:11px; padding:5px 12px;")),
          plotOutput(ns("fp_filtered"), height = "calc(100vh - 360px)")
        )
      )
    ),
    ...
    tabPanel("Sync",
      layout_columns(
        ...
        card(
          full_screen = TRUE,
          card_header("US Sectors — Regime Sync",
                      style = paste0("background:", DARK_HDR, "; color:", TEXT_DIM,
                                     "; font-size:11px; padding:5px 12px;")),
          plotOutput(ns("sync_sector"), height = "calc(100vh - 360px)")
        ),
        card(
          full_screen = TRUE,
          card_header(textOutput(ns("sync_filtered_title"), inline = TRUE),
                      style = paste0("background:", DARK_HDR, "; color:", TEXT_DIM,
                                     "; font-size:11px; padding:5px 12px;")),
          plotOutput(ns("sync_filtered"), height = "calc(100vh - 360px)")
        )
      )
    )
  )
...
mod_regime_server <- function(id, filtered, relative_mode, bmk) {
  moduleServer(id, function(input, output, session) {
    ...
    # ── Fingerprint: fixed sector tickers (always US sectors) ────────────────
    sector_syms <- intersect(g_sector_tickers, colnames(xts_ret_shiny))

    output$fp_sector <- renderPlot({
      validate(need(length(sector_syms) >= 2, "Sector tickers not in xts_ret_shiny"))
      plot_regime_fingerprint_box(
        xts_ret = xts_ret_shiny,
        tickers = sector_syms,
        rt      = rt_shiny,
        title   = "US Sectors \u2014 \u03b1 vs SPY across Full Cycle"
      )
    }, bg = "white")

    # ── Fingerprint: filtered tickers (follows sidebar mode) ─────────────────
    output$fp_filtered_title <- renderText({
      paste0("Filtered Universe (", nrow(filtered()), " tickers) \u2014 Regime Fingerprint")
    })

    output$fp_filtered <- renderPlot({
      syms <- filtered()$symbol
      syms <- syms[syms %in% colnames(xts_ret_shiny)]
      syms <- syms[syms != "SPY"]
      validate(need(length(syms) >= 2, "Need at least 2 tickers in xts_ret_shiny"))
      plot_regime_fingerprint_box(
        xts_ret = xts_ret_shiny,
        tickers = syms,
        rt      = rt_shiny
    ...
    # ── Sync: fixed sector tickers ────────────────────────────────────────────
    output$sync_sector <- renderPlot({
      validate(need(length(sector_syms) >= 2, "Sector tickers not in xts_ret_shiny"))
      plot_regime_sync(
        xts_ret = xts_ret_shiny,
        tickers = sector_syms,
    ...
    output$sync_filtered_title <- renderText({
      paste0("Filtered Universe (", nrow(filtered()), " tickers) \u2014 Regime Sync")
    })

    output$sync_filtered <- renderPlot({
      syms <- filtered()$symbol
      syms <- syms[syms %in% colnames(xts_ret_shiny)]
      syms <- syms[syms != "SPY"]
      validate(need(length(syms) >= 2, "Need at least 2 tickers in xts_ret_shiny"))
