################################################################################
# CHART TEMPLATE : ma_price
# FILE           : key_plots/chart_ma_price.R
#
# FUNCTION
#   plot_ma_chart(ticker, xts_price, from_date, ma1, ma2, t_fall)
#   → patchwork: price + MA panel / drawdown panel
#   → y-axis shows actual adjusted price ($)
#
# SEE ALSO
#   key_plots/chart_ma_cumret.R  — same chart using cumulative return (wealth index)
#
# INVOKE
#   source(here("key_plots/chart_ma_price.R"))
#   plot_ma_chart("URTH", xts_price)
#   plot_ma_chart("SPY",  xts_price, from_date = "2022-01-01", ma1 = 50, ma2 = 200)
################################################################################
...
plot_ma_chart <- function(
  ...
  if (!ticker %in% colnames(xts_price))
    stop("ticker '", ticker, "' not found in xts_price")

  df <- tibble(
    date  = as.Date(index(xts_price)),
    price = as.numeric(coredata(xts_price[, ticker]))
  ) %>%
    mutate(
  ...
  p1 <- ggplot(df, aes(x = date)) +
    ...
    labs(
      title    = paste0(ticker, "  \u2014  Price vs ", ma1, "DMA & ", ma2, "DMA"),
      subtitle = sprintf(
...
if (!isTRUE(getOption("knitr.in.progress"))) {
  library(here)
  if (!exists("xts_price"))
    stop("xts_price not found — load adjusted prices before sourcing this script")

  print(plot_ma_chart("URTH", xts_price))
}
